...
describe('Event API Routes', () => {
  ...
  describe('GET /api/events', () => {
    it('should return all events', async () => {
      ...
      expect(response.body.some((e: Event) => e.userId === testUser.id)).toBe(true);
      expect(response.body.some((e: Event) => e.userId === anotherUser.id)).toBe(true);
    });
  ...
  describe('GET /api/users/:userId/events', () => {
    it('should return all events for a specific user', async () => {
      ...
      const response = await request(app)
        .get(`/api/users/${testUser.id}/events`);
      ...
      expect(response.body.some((e: Event) => e.title === event1.title)).toBe(true);
      expect(response.body.some((e: Event) => e.title === event3.title)).toBe(true);
    });
